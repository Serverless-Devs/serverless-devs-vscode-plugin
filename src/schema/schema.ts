...
// 应用所依赖组件的schema
export async function getCmptSchema(componentName: string): Promise<any> {
  const cmptBaseSchema: any = baseSchema;
  const cmptContent: any = await fetch('https://registry.devsapp.cn/package/content', {
    method: 'POST',
    headers: {
  ...
  if (!_.isEmpty(properties)) {
    ...
    cmptBaseSchema.properties.services.patternProperties['^[a-zA-Z0-9-]*$'].properties.props =
      properties;
  } else {
